...
function EditPropListing() {
  ...
  // ! Redirect if user not listing
  useEffect(() => {
    if (listing && listing.userRef !== auth.currentUser.uid) {
      toast.error('You have no access to edit the listing')
      navigate('/')
    }
  })

  // ! Use effect for edit listing
  useEffect(() => {
    setLoading(true)
    // ! Fetch data listings from the return using async
    const fetchListing = async () => {
      ...
      if (docData.exists()) {
        ...
        // ! auto populate the field with its current doc data
  ...
  // ! Getting user data list and adding to the form DB
  useEffect(() => {
    if (isMounted) {
  ...
  // ! Forwarding the submit form request using async to DB
  const onSubmit = async (e) => {
    ...
    if (geolocationEnabled) {
      ...
      // ! Store the fetch data from API results
      const data = await response.json()
      // ! If no return, set to 0 for latitude; ? to check if it's null or not
      ...
      if (location === undefined || location.includes('undefined')) {
        setLoading(false)
        toast.error('Incorrect address, please try again')
        return
      }
    } else {
    ...
    // ! Call function for the images uploaded
    // ! Array of image URL
    ...
    // ! Updating the doc - collection listing
    const docRef = doc(db, 'listings', params.listingId)
  ...
  const onMutate = (e) => {
    ...
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...prevState,
        // ! If target is not a boolean / will select and enable to true
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className='formPriceDiv'>
            <input
              ...
              required
            />
            {type === 'rent' && <p className='formPriceText'>PHP / Month</p>}
          </div>
